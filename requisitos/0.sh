...
if [ -d requisitos/theHarvester ]
	then
		sudo rm -r theHarvester && sudo git clone https://github.com/laramies/theHarvester.git && cd theHarvester && sudo chmod 777 requirements.txt && sudo python3 -m pip install -r requirements.txt && sudo pip install aiohttp && sudo pip install aiodns && python3 setup.py install && cd ..
	else 
		sudo git clone https://github.com/laramies/theHarvester.git && cd theHarvester && sudo chmod 777 requirements.txt && sudo python3 -m pip install -r requirements.txt && sudo pip install aiohttp && sudo pip install aiodns && python3 setup.py install && cd ..
